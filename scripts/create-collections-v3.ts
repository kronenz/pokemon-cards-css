/**
 * PocketBase Collections Setup Script v3
 * JSON 필드 옵션 수정
 */
...
async function createCollections() {
  ...
  try {
    ...
    if (existingCards) {
      ...
      const cardsResult = await pb.collections.create({
        ...
        schema: [
          ...
          { name: 'image', type: 'url', options: {} },
          { name: 'rarity', type: 'text', options: {} },
          { name: 'team', type: 'text', options: {} },
          { name: 'effect_type', type: 'text', options: {} },
          { name: 'tags', type: 'json', options: { maxSize: 10000 } },
          { name: 'is_shared', type: 'bool', options: {} },
          { name: 'like_count', type: 'number', options: { min: 0, noDecimal: true } },
          { name: 'comment_count', type: 'number', options: { min: 0, noDecimal: true } },
          { name: 'view_count', type: 'number', options: { min: 0, noDecimal: true } },
          { name: 'share_count', type: 'number', options: { min: 0, noDecimal: true } },
          { name: 'holographic_effect', type: 'json', options: { maxSize: 10000 } }
        ],
        listRule: '',
        viewRule: '',
        createRule: '',
    ...
    console.log('  ❌ cards:', JSON.stringify(e.response?.data || e.message));
  }
  ...
  try {
    ...
    console.log('  ❌ comments:', JSON.stringify(e.response?.data || e.message));
  }
  ...
  try {
    ...
    console.log('  ❌ likes:', JSON.stringify(e.response?.data || e.message));
  }
  ...
  try {
    ...
    if (existingFeed) {
      ...
      await pb.collections.create({
        ...
        schema: [
          ...
          { name: 'content', type: 'json', options: { maxSize: 50000 } },
          { name: 'metadata', type: 'json', options: { maxSize: 50000 } }
        ],
        listRule: '',
        viewRule: '',
        createRule: '',
    ...
    console.log('  ❌ feed_items:', JSON.stringify(e.response?.data || e.message));
  }
  ...
  try {
    ...
    if (existingNotif) {
      ...
      await pb.collections.create({
        ...
        schema: [
          { name: 'user', type: 'relation', required: true, options: { collectionId: usersCollectionId, maxSelect: 1, cascadeDelete: false } },
          { name: 'type', type: 'text', required: true, options: { max: 50 } },
          { name: 'data', type: 'json', options: { maxSize: 50000 } },
          { name: 'is_read', type: 'bool', options: {} }
        ],
        listRule: '',
        viewRule: '',
        createRule: '',
    ...
    console.log('  ❌ notifications:', JSON.stringify(e.response?.data || e.message));
  }
